...
function getPayloadConfigFromPayload(
  ...
  const itemPayload = (payload as { payload?: Record<string, unknown> }).payload

  if (itemPayload && typeof itemPayload === "object") {
    const name = typeof itemPayload.name === "string" ? itemPayload.name : undefined
    ...
    return (
      ...
      >
        {payload
          ...
          .map((item) => {
            const itemName = "name" in item ? String(item.name ?? "") : ""
            const key = `${nameKey || item.dataKey || itemName || "value"}`
            const itemConfig = getPayloadConfigFromPayload(config, item, key)
            ...
            return (
              ...
              >
                {!hideIcon && (
                  <div
                    className={cn(
                      "h-3 w-3 shrink-0 rounded-[4px] border border-border/50",
                      itemConfig?.theme ? "bg-[--color-value]" : "bg-[--color-value]"
                    )}
                  >
